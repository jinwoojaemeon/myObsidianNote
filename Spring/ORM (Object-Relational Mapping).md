...
"개발자는 **'자바(Java)'**라는 언어를 쓰고, 데이터베이스는 **'SQL'** 이라는 언어를 씁니다. 서로 사용하는 언어가 달라, 서로 통하지 않는 관계라고 생각했을 때

**ORM은 이 둘 사이의 '동시통역사'입니다.**

통역사(ORM)가 있으면 제가 자바로만 말해도 알아서 SQL로 번역해 DB에 전달해 줍니다. 덕분에 저는 통역에 신경 쓸 필요 없이 제 할 말(비즈니스 로직)에만 집중할 수 있게 해주는 기술입니다."
