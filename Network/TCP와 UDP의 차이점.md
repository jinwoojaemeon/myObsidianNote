...
**TCP(Transmission Control Protocol)**는 전송 계층에서 사용되는 **연결 지향형(Connection-oriented)**, **신뢰성 있는(Reliable)** 프로토콜입니다.

### **TCP의 주요 특징**

1. **연결 지향형 (Connection-oriented)**
   - 통신 전에 연결을 설정 (3-way handshake)
   - 통신 후 연결을 해제 (4-way handshake)
...
**UDP(User Datagram Protocol)**는 전송 계층에서 사용되는 **비연결형(Connectionless)**, **비신뢰성(Unreliable)** 프로토콜입니다.
...
- ✅ 일부 데이터 손실이 허용되는 경우
- ✅ **빠른 응답**이 필요한 경우
- ✅ **멀티캐스트/브로드캐스트**가 필요한 경우
- ✅ **오버헤드**를 최소화해야 하는 경우

**예시**: 동영상 스트리밍, 온라인 게임, DNS 조회
...
- **선택 기준**: 데이터 정확성이 중요하면 TCP, 속도와 실시간성이 중요하면 UDP를 선택합니다.
...
#### 1. TCP의 3-way handshake 과정은?

**3-way handshake**는 TCP 연결을 설정하는 과정입니다:
...
#### 2. TCP의 흐름 제어(Flow Control)는 어떻게 동작하나?

**흐름 제어**는 수신자의 버퍼 크기에 맞춰 송신 속도를 조절하는 메커니즘입니다.
...
#### 3. TCP의 혼잡 제어(Congestion Control)는 어떻게 동작하나?

**혼잡 제어**는 네트워크 혼잡 상황을 감지하고 전송 속도를 조절하는 메커니즘입니다.
...
- 예: QUIC 프로토콜 (UDP 기반, TCP의 신뢰성 기능 포함)
